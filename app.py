...
from janitor.ai import AIUnavailable, summarize_scan
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/ai-summary":
            try:
                ...
                if not scan:
                    raise InputError("找不到对应的扫描记录")
                return json_response(self, 200, {"ok": True, "scan_id": scan["scan_id"], "ai": summarize_scan(scan), "thinking": "disabled"})
            except json.JSONDecodeError:
                return json_response(self, 400, {"ok": False, "error": "请求内容不是有效的 JSON"})
            except InputError as exc:
                return json_response(self, 400, {"ok": False, "error": str(exc)})
                ...
                return json_response(self, 500, {"ok": False, "error": "AI 解读暂时无法生成"})
